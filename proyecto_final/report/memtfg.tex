...
\bottomrule
\end{tabular}
\caption{Conjuntos de datos utilizados. Los cuatro proceden del ISTAC y cubren la provincia de Santa Cruz de Tenerife (código INE: 38).}
\end{table}
...
\begin{itemize}
  \item Datos: tabla con un registro por municipio; columnas municipio (cadena) y renta\_media (numérico, euros). Fichero fuente: rentamedia-sc-3.csv.
  \item Mapeos:
    \begin{itemize}
...
\begin{itemize}
  \item Datos: tabla con un registro por sección censal; columna renta\_neta. Fichero fuente: rentamedia-sc-3.csv.
...
\begin{itemize}
  \item Datos: cinco filas, una por fuente de ingresos; columnas fuente y pct (porcentaje del total). Fichero fuente: distribucion-renta-ingresos.csv.
...
\begin{itemize}
  \item Datos: tabla con un registro por categoría de ocupación; columnas ocupacion, desviacion (porcentaje de mujeres menos 50) y mayoria ("Más mujeres" / "Más hombres"). Fichero fuente: ocupacion-sc-3.csv.
  \item Mapeos:
    \begin{itemize}
...
\begin{itemize}
  \item Datos: un registro por sección censal con columnas municipio e isla. Se filtra por isla y se genera un PNG independiente por isla. Fichero fuente: rentamedia-sc-3.csv.
...
\begin{itemize}
  \item Datos: un registro por municipio; columnas pct\_elementales (porcentaje de trabajadores con nivel formativo elemental) y renta\_neta (renta media). Ficheros fuente: rentamedia-sc-3.csv, ocupacion-sc-3.csv.
...
\begin{itemize}
  \item Datos: tabla con columnas quintil (1--5), actividad (sector CNAE) y pct (porcentaje de trabajadores). Ficheros fuente: rentamedia-sc-3.csv, actividad-sc-3.csv.
...
\begin{itemize}
  \item Datos: tabla con columnas municipio, año (2021, 2022, 2023) y renta\_norm (renta normalizada en [0,1] para poder comparar dentro del mismo municipio a lo largo del tiempo). Fichero fuente: rentamedia-sc-3.csv.
...
\begin{itemize}
  \item Renta neta media por hogar (euros), procedente de la Estadística de Distribución de Renta de los Hogares del ISTAC (2023).
...
\addlinespace
Waterfall fuentes & Continuidad, Parte-Todo & Las barras encadenadas crean una línea de acumulación continua que comunica la composición del total, donde cada segmento es una parte del 100\%. \\
\addlinespace
...
\begin{enumerate}
  \item Carga (*\_load): lectura de los datos crudos desde ficheros CSV descargados del ISTAC.
