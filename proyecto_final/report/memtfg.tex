...
\begin{center}
  {\Huge Trabajo Final de Asignatura}\\[5mm]
  {\large Máster Universitario en Inteligencia de Datos y Ciberseguridad}
\end{center}

\HRule
\begin{flushright}
        {\Huge Visualización\\[2mm]Análisis de la desigualdad económica\\en la provincia de Santa Cruz de Tenerife} \\[2.5mm]
...
La provincia de Santa Cruz de Tenerife, que comprende las islas de Tenerife, La Palma, La Gomera y El Hierro, presenta una distribución de la renta notablemente desigual: municipios turísticos y de servicios conviven con zonas de actividad agrícola o poca actividad económica cuya renta per cápita es significativamente inferior. Este proyecto aplica las técnicas de visualización de datos aprendidas en la asignatura para explorar, comunicar y cuantificar esa desigualdad a través de datos oficiales del Instituto Nacional de Estadística (INE).

\section{Fuentes de datos}

El proyecto emplea cuatro conjuntos de datos, todos de acceso público mediante la API JSON-STAT del INE:
...
Este capítulo analiza cada visualización generada siguiendo la gramática de gráficos de Wilkinson \cite{wilkinson2005} tal como la implementa Plotnine \cite{plotnine}: datos, mapeos estéticos (aes), geometrías (geom\_*), escalas, coordenadas, facetas y tema.
...
El lollipop es preferible al gráfico de barras cuando hay muchas categorías: la línea delgada reduce la tinta y la comparación de valores se realiza rápidamente gracias al punto claramente posicionado.
...
\section{Slope chart: evolución de la renta 2021--2023}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{images/slope}
\caption{Slope chart de evolución de la renta media por municipio entre 2021 y 2023.}
\end{figure}

\begin{itemize}
  \item Datos: tabla con un registro por municipio; columnas municipio, renta\_2021 y renta\_2023.
  \item Mapeos: x = año (dos valores: 2021 y 2023), y = renta\_media, group = municipio, color = isla.
  \item Geometrías: geom\_line (la pendiente entre los dos años) + geom\_point en los extremos.
  \item Escalas: eje Y continuo en euros; eje X discreto con dos posiciones.
  \item Coordenadas: cartesianas.
  \item Facetas: ninguna; todos los municipios superpuestos.
  \item Tema: theme\_minimal().
\end{itemize}

El slope chart es ideal para comparar el cambio relativo entre dos momentos: la inclinación de cada línea indica si un municipio ha ganado o perdido posición relativa. La densidad de líneas cruzadas señala cambios de orden en el ranking.
...
El mapa es un mapa coroplético (choropleth): cada polígono del territorio se colorea proporcionalmente al valor de una variable cuantitativa. En este caso los polígonos son las secciones censales de la provincia de Santa Cruz de Tenerife y la variable es la renta neta media por hogar (OBS\_VALUE).

Los datos geométricos provienen de los ficheros GeoJSON del Instituto Geográfico Nacional (IGN), que delimitan las secciones censales a nivel de provincia. Estos polígonos se unen al dataframe de renta mediante el código de sección censal de siete dígitos, que identifica de forma única cada sección en España.

La librería Folium genera el mapa como un documento HTML autónomo que envuelve una instancia de Leaflet.js. Leaflet renderiza los polígonos en un canvas SVG sobre un tile base (OpenStreetMap por defecto) y gestiona las interacciones: zoom, paneo y hover sobre los polígonos.

\subsection{Codificación visual}

La variable de renta se mapea a color mediante una escala continua lineal (branca.LinearColormap) con la paleta YlOrRd (amarillo a rojo), donde el amarillo más claro corresponde a las secciones de menor renta y el rojo más oscuro a las de mayor renta. La escala se ajusta a los percentiles 5 y 95 de la distribución para reducir el efecto de valores atípicos en los extremos y maximizar el contraste en la zona central de la distribución.

Cada polígono incluye un tooltip que muestra el nombre del municipio, el código de sección censal y el valor de renta al pasar el ratón por encima, lo que permite identificar zonas de interés sin necesidad de una leyenda exhaustiva.
...
\begin{itemize}
  \item Figura-Fondo: los polígonos coloreados destacan sobre el tile base neutro.
  \item Punto Focal: los clusters de color rojo intenso (zonas de alta renta) atraen la atención de inmediato y permiten identificar las áreas más ricas de la provincia.
...
\addlinespace
Slope chart & Cambio Temporal, Destino Común & Las líneas paralelas comunican continuidad temporal; las que se cruzan señalan cambios de orden. \\
\addlinespace
...
\section{Recomendaciones de diseño aplicadas}

A continuación se describen las recomendaciones de diseño más relevantes que se siguieron en el proyecto:

\begin{enumerate}

  \item Relación datos/tinta (data-ink ratio).
  Todos los gráficos emplean theme\_minimal() de Plotnine, que elimina el fondo gris, los bordes del panel y las rejillas secundarias. En los lollipops se usa una línea fina en lugar de barras rellenas para maximizar este ratio.

  \item Ordenación por valor, no alfabética.
  En el ranking de municipios, el heatmap y los box plots, las categorías del eje Y se ordenan por la magnitud de la variable cuantitativa (renta mediana o media). Esto facilita la lectura comparativa y evita la búsqueda aleatoria.

  \item Uso de coord\_flip() para etiquetas largas.
  Los gráficos con etiquetas de municipio u ocupación se orientan horizontalmente mediante coord\_flip() para evitar la rotación de texto y reducir la carga cognitiva del lector.

  \item Paletas semánticas.
  \begin{itemize}
    \item El diverging bar de género usa azul (mayoría masculina) y rojo (mayoría femenina), colores con convención social reconocible.
    \item El grouped bar de ocupación usa verde (ocupaciones de alta renta) y rojo (ocupaciones elementales) siguiendo la convención semafórica.
    \item El heatmap usa la paleta RdYlGn: rojo para valores bajos (situación desfavorable) y verde para altos.
  \end{itemize}

  \item Anotaciones y líneas de referencia.
  El diverging bar incluye una línea en y=0 (paridad exacta). El scatter incluye la recta de regresión. El histograma incluye una línea vertical en la mediana. Estas anotaciones anclan la interpretación sin necesidad de texto adicional.

  \item Carga cognitiva limitada en variables categóricas.
  Ningún gráfico supera 10 categorías de color simultáneas (principio de límite de la memoria de trabajo). El check de activos test\_ocupacion\_categorias verifica programáticamente este límite.

  \item Generación de archivos separados por subgrupo.
  Los box plots se generan como cuatro archivos PNG independientes (uno por isla) en lugar de un único panel con facet\_wrap. Esto permite ajustar la escala vertical a cada isla y mantener la legibilidad de las etiquetas de municipio.
...
\begin{itemize}
  \item Reproducibilidad: dado el mismo conjunto de datos y el mismo prompt, el pipeline siempre produce el mismo PNG.
  \item Trazabilidad: Dagster registra metadatos de cada ejecución: tamaño del archivo generado, número de registros del dataframe de entrada, ruta del archivo, etc.
  \item Modularidad: cada step es independiente. Cambiar la transformación de datos de data\_renta\_municipio sólo implica re-ejecutar ese asset y los que dependen de él; los demás no se recalculan.
  \item Observabilidad: la interfaz web de Dagster muestra el grafo de dependencias, el historial de ejecuciones y los metadatos de cada asset en tiempo real.
\end{itemize}
...
test\_slope\_dos\_anios & El slope chart tiene columnas renta\_2021 y renta\_2023 y >= 40 municipios \\
...
\section{Metadatos de los checks}

Todos los checks incluyen metadatos estructurados que Dagster almacena en su base de datos de eventos:
\begin{itemize}
  \item description: descripción legible del criterio de aceptación.
  \item gestalt: principio de diseño relacionado (por ejemplo, "Figura-Fondo", "Continuidad", "Parte-Todo").
  \item Métricas numéricas específicas (n\_municipios, correlacion, ratio\_max\_min, etc.) que permiten monitorizar la evolución del dataset a lo largo del tiempo.
\end{itemize}
...
Los gráficos estáticos en PNG son adecuados para un informe impreso o un documento PDF, pero no permiten la exploración libre de los datos ni la combinación narrativa de todas las visualizaciones en un único flujo de lectura. El dashboard de Streamlit (dashboard.py) cubre esa necesidad: agrupa los catorce gráficos del proyecto junto con el mapa interactivo, añade texto explicativo contextual y organiza el contenido en una secuencia que sigue el arco narrativo del análisis.
...
\addlinespace
Desigualdad y género & Slope chart, diverging bar & ?`Ha cambiado la brecha? ?`Se reparte igual el trabajo precario? \\
\addlinespace
...
\begin{itemize}
  \item Tipo de gráfico y justificación: se indica el tipo de geometría (geom\_*) y por qué es adecuado para los datos (por ejemplo, "un lollipop reduce la tinta sin perder precisión respecto a un gráfico de barras").
...
Principios de gestalt: Figura-Fondo (línea delgada reduce tinta),
